...
def batch_group():
    """Manage multiple tasks in batches."""
    pass

@batch_group.command(name="create")
@click.argument("batch_file", type=click.Path(exists=True))
@click.option("--project-dir", default=".", help="Project directory")
def batch_create(batch_file, project_dir):
    """Create multiple tasks from a batch JSON file."""
    handle_batch_create_command(batch_file, project_dir)

@batch_group.command(name="status")
@click.option("--project-dir", default=".", help="Project directory")
def batch_status(project_dir):
    """Show status of all specs in project."""
    handle_batch_status_command(project_dir)

@batch_group.command(name="cleanup")
@click.option("--project-dir", default=".", help="Project directory")
@click.option("--dry-run/--no-dry-run", default=True, help="Dry run mode")
def batch_cleanup(project_dir, dry_run):
    """Clean up completed specs and worktrees."""
    handle_batch_cleanup_command(project_dir, dry_run)
...
def handle_batch_create_command(batch_file: str, project_dir: str) -> bool:
    ...
    next_id = (
        max([int(s.split("-")[0]) for s in existing_specs if s and s[0].isdigit()] or [0]) + 1
    )
    ...
    for idx, task in enumerate(tasks, 1):
        ...
        requirements = {
            ...
            "inferred_from": {},
            "created_at": str(datetime.now().isoformat()) if 'datetime' in globals() else "now",
            "estimate": {
        ...
        for spec_name in completed:
            ...
            if wt_path.exists():
                try:
                    ...
                    shutil.rmtree(wt_path, ignore_errors=True)
                    print_status(
                        f"Failed to remove worktree {spec_name}: {e}. Directory removed manually.", "warning"
                    )
